...
uint8_t affineTrans(uint8_t inv){
	uint8_t return_value = 0;
	int i,j;
...
void subBytes_16(uint8_t *input, uint8_t mx){
	int i;
	for(i=0;i<16;i++)
...
void subBytes_4(uint8_t *input, uint8_t mx){
	int i;
	for(i=0;i<4;i++)
		input[i] = affineTrans(GF256_inv(input[i], mx));
	return;
}
void subBytes_4_inverse(uint8_t *input, uint8_t mx){
	int i;
	for(i=0;i<4;i++)
		input[i] = GF256_inv(affineTrans_inverse(input[i]), mx);
	return;
...
void mixColumn(uint8_t *input, uint8_t mx){
	int i,j,k;
	uint8_t input_array[4][4] = {0};
	for(i=0;i<4;i++){
		for(j=0;j<4;j++)
...
void keyExpansion(uint8_t *key, uint8_t mx, uint8_t **output){
...
uint8_t* extractRoundKey(uint8_t **word, int Round){
